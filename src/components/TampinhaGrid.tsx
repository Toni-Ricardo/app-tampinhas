...
/* 🛠️ Inputs: BORDA NEON + FUNDO SÓLIDO */
const inputClass =
  'w-full h-10 rounded-lg neon-border-cyan bg-[#0b0f19] px-4 text-sm text-slate-100 outline-none transition-all placeholder:text-tr-muted/50 focus:border-amber-500 focus:ring-1 focus:ring-amber-500/30 uppercase tracking-wide'

/* ✅ RÓTULOS: Laranja + sem negrito */
...
function BotaoOrigem({ label, ativo, onClick }: { label: Origem; ativo: boolean; onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`group flex flex-1 items-center justify-center gap-2.5 rounded-lg neon-border-cyan bg-[#0b0f19] h-10 text-[13px] font-semibold tracking-wide transition-all duration-200 active:scale-[0.98] ${
...
export function NovaTampinhaModal({ open, onClose, onSubmit }: NovaTampinhaModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* ✅ CAIXA EXTERNA = EFEITO GLASS */}
      <div className="relative z-10 w-full max-w-md glass-panel rounded-2xl p-6 overflow-hidden">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <section>
              ...
              {/* ✅ ÁREA DE UPLOAD = BORDA NEON + FUNDO SÓLIDO */}
              <button
                type="button"
                onClick={() => inputFotoRef.current?.click()}
                className="group flex w-full h-32 flex-col items-center justify-center gap-2 rounded-lg neon-border-cyan bg-[#0b0f19] transition-all duration-300"
              >
                {preview ? (
            ...
            <div className="flex items-center gap-3 pt-4 mt-8">
              <button 
                type="button" 
                onClick={onClose} 
                className="flex-1 h-10 inline-flex items-center justify-center rounded-lg neon-border-cyan bg-[#0b0f19] text-xs font-normal uppercase text-tr-muted hover:text-slate-200"
              >
                Cancelar
              </button>
              <button 
                type="submit" 
                disabled={enviando} 
                className="flex-1 h-10 inline-flex items-center justify-center rounded-lg neon-border-cyan bg-[#0b0f19] text-xs font-normal uppercase text-amber-500 hover:text-amber-400"
              >
